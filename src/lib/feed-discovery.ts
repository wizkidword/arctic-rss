...
export async function discoverFeedFromUrl(input: string): Promise<DiscoveredFeed> {
  const startUrl = normalizeHttpUrl(input)
  const firstResponse = await safeFetchText(startUrl)
  ...
  const candidates = extractFeedCandidatesFromHtml(
    firstResponse.text,
    firstResponse.url.href
  )

  for (const candidate of candidates) {
    try {
      const candidateUrl = normalizeHttpUrl(candidate)
      const response = await safeFetchText(candidateUrl)
