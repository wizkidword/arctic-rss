...
describe("FeedDirectorySubscribeDialogContent", () => {
  ...
  it("lets readers name a new folder while subscribing", async () => {
    const user = userEvent.setup()
    ...
    const dialog = screen.getByRole("dialog", {
      name: "Subscribe to NPR - National",
    })
    await user.selectOptions(
      within(dialog).getByRole("combobox", { name: "Folder" }),
      "new-folder"
    )
    await user.type(
      within(dialog).getByRole("textbox", { name: "New folder name" }),
      "Tech Watch"
    )
    await user.click(within(dialog).getByRole("button", { name: "Subscribe" }))

    await waitFor(() => {
      expect(subscribeDirectoryFeedAction).toHaveBeenCalled()
    })

    const submittedFormData = subscribeDirectoryFeedAction.mock.calls[0]?.[1]
    expect(submittedFormData).toBeInstanceOf(FormData)
    expect((submittedFormData as FormData).get("folderId")).toBe("")
    expect((submittedFormData as FormData).get("folderName")).toBe("Tech Watch")
  })
})
...
describe("FeedDirectorySubscribeButton", () => {
  ...
  it("can render a feed-specific Follow trigger for directory cards", async () => {
    const user = userEvent.setup()
  ...
  it("greys out feed follows for guest browsing without opening the picker", async () => {
    const user = userEvent.setup()
    ...
    render(
      <FeedDirectorySubscribeButton
        ...
        feedLabel="NPR - National"
        folders={folders}
        readOnlyReason="Create an account to follow feeds."
        subscribed={false}
        triggerIcon="plus"
        triggerLabel="Follow"
      />
    ...
    expect(followButton).toHaveProperty("disabled", true)
    expect(followButton.getAttribute("title")).toBe(
      "Create an account to follow feeds."
    )

    await user.click(followButton)

    expect(screen.queryByRole("dialog")).toBeNull()
    expect(subscribeDirectoryFeedAction).not.toHaveBeenCalled()
  })
