      ...
      <div className="mx-auto flex max-w-[1600px] flex-col gap-4 p-3 sm:p-5 lg:p-6">
        ...
        <section className="overflow-hidden rounded-lg border bg-card">
          <SectionHeader
            badge="OPML"
            icon={FileUpIcon}
            title="Import OPML into Discover"
          />
        ...
        <section className="overflow-hidden rounded-lg border bg-card">
          <SectionHeader
            badge="Reddit"
            icon={HashIcon}
            title="Add Subreddit to Discover"
          />
        ...
        <section className="overflow-hidden rounded-lg border bg-card">
          <SectionHeader
            badge={`${discoverCategories.length} cards`}
            icon={PaletteIcon}
            title="Edit Discover Cards"
          />
          <AdminDiscoverCategoryMetadataForm categories={discoverCategories} />
        </section>
        ...
        <section className="overflow-hidden rounded-lg border bg-card">
          <SectionHeader
            badge={`${dashboard.bugReports.length} shown`}
            icon={BugIcon}
            title="Bug reports"
          />
          {dashboard.bugReports.length ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] text-left text-sm">
                <thead className="border-b bg-muted/40 text-xs text-muted-foreground">
                  <tr>
                    <TableHeading>Report</TableHeading>
                    <TableHeading>Reader</TableHeading>
                    <TableHeading>Page</TableHeading>
                    <TableHeading>Browser</TableHeading>
                    <TableHeading>Status</TableHeading>
                    <TableHeading>Sent</TableHeading>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {dashboard.bugReports.map((report) => (
                    <tr key={report.id}>
                      ...
                      <TableCell>
                        ...
                        {report.contactEmail &&
                          report.contactEmail !== report.userEmail && (
                            <p className="mt-0.5 text-xs text-muted-foreground">
                              {report.contactEmail}
                            </p>
                          )}
            ...
            <EmptyState icon={BugIcon} message="No bug reports" />
          )}
        </section>

        <section className="overflow-hidden rounded-lg border bg-card">
          <SectionHeader
            badge={`${dashboard.featureSuggestions.length} shown`}
            icon={LightbulbIcon}
            title="Feature suggestions"
          />
          {dashboard.featureSuggestions.length ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] text-left text-sm">
                <thead className="border-b bg-muted/40 text-xs text-muted-foreground">
                  <tr>
                    <TableHeading>Suggestion</TableHeading>
                    <TableHeading>Reader</TableHeading>
                    <TableHeading>Page</TableHeading>
                    <TableHeading>Browser</TableHeading>
                    <TableHeading>Status</TableHeading>
                ...
                <tbody className="divide-y">
                  {dashboard.featureSuggestions.map((suggestion) => (
                    <tr key={suggestion.id}>
                      ...
                      <TableCell>
                        <p>{suggestion.userEmail}</p>
                        {suggestion.contactEmail &&
                          suggestion.contactEmail !== suggestion.userEmail && (
                            <p className="mt-0.5 text-xs text-muted-foreground">
                              {suggestion.contactEmail}
                            </p>
                          )}
                      ...
                      <TableCell className="max-w-60">
                        <span className="line-clamp-2 text-xs text-muted-foreground">
                          {suggestion.userAgent || "Not captured"}
                        </span>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {dateFormatter.format(suggestion.createdAt)}
                      </TableCell>
                    </tr>
                  ))}
                </tbody>
            ...
            <EmptyState
              icon={LightbulbIcon}
              message="No feature suggestions"
            />
          )}
        </section>
