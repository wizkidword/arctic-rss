...
| MOB-OFFLINE-001 | IN_PROGRESS | Phase 4 source adds mandatory SQLite user/device owner metadata and purges unowned or switched-account data before sync. Android source disables backup and has a local native-config check; signed-manifest/restore proof, authenticated route grouping, corrupt-row recovery, and full restore/account-switch tests remain open. |
