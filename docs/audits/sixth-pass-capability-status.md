...
| MOB-SYNC-001 | IN_PROGRESS | Phase 5 now refuses to advance the cursor or first-sync milestone for non-empty generic event pages, preventing loss until typed transactional application exists. Typed event variants, cache invalidation, truthful bootstrap/full resync, retention maintenance, and disposable-PostgreSQL convergence evidence remain open. |
