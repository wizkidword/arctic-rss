...
| MOB-OFFLINE-001 | IN_PROGRESS | Phase 4 source adds mandatory SQLite user/device owner metadata and purges unowned or switched-account data before sync. A hidden authenticated route group prevents protected screens from mounting while signed out and only restores recognized in-app paths after sign-in. Android source disables backup and has a local native-config check; signed-manifest/restore proof, corrupt-row recovery, and full restore/account-switch tests remain open. |
