...
| Phase 9 — search measurement | Implemented | A stored weighted `tsvector` and GIN index remove repeated document construction; the loopback-only benchmark records representative `EXPLAIN (ANALYZE, BUFFERS)` evidence and a 350 ms p95 target. |
