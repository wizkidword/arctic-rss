...
| FP-002 | Initial implementation: expand-only entry lease fields, `PROCESSING` status, atomic `FOR UPDATE SKIP LOCKED` claims, attempt/owner/expiry fencing, and parent counters updated only by a successful fenced terminal transition. Completion requires no pending or processing entries. | Lease unit tests plus PostgreSQL concurrent-claim, reclaim, stale-finalization, cancellation, counter, and completion checks; BullMQ lost-lock/Redis and separate-process replay evidence remain pending | Not run | All 45 migrations applied to a disposable PostgreSQL 17.10 fixture; OPML enum and active-claim index verified. | Yes — enum, lease metadata, and active-claim index; production ready false | Not deployed | No | None | Complete Redis/worker replay evidence and approved release verification |
