...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FP-001 | Current-record eligibility now gates monitors, Smart Digest processing/email, and AI-digest provider calls. Disablement pauses rules and cancels retryable Smart Digest and AI-digest work. Chat bot delivery is room-owned system automation (`senderUserId: null`), so no account-owner eligibility check applies; user-owned OPML import leases remain Phase 2 work. | Policy plus disable-before-claim, disable-during-processing, Smart Digest canceled-run, and AI-digest pre-provider recheck coverage; full suite passes (284 files, 1,400 tests) | Not run this pass | All 44 migrations, including both additive `CANCELED` values, applied to a disposable PostgreSQL 17.10 fixture; migration status and enum labels verified. Background-worker fixture remains pending. | Yes — additive `CANCELED` enum values; production ready false | Not deployed | No | Required before device sessions | Begin Phase 2 OPML lease design; release remains `DEPLOY <short-sha>` gated |
