...
| Offline/sync/queue | Local source and real sync tests cover ownership, non-lossy cursor/bootstrap, retention, and queue conflicts. The injected SQLite-store harness covers account switching, failed owner transfer, and interrupted replay with bounded backoff. | Source verified only | Native SQLite/runtime account-switch/restore proof and signed-device evidence |
